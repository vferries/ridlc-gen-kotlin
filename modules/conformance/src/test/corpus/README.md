...
| `kt-values`  | this repository                                                      | a same-package pattern, a regex constant, a float step from a negative minimum, inline scalar constraints, and `Probe`, an interface whose `require` and `ensure` can both fail |
