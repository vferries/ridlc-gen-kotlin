...
class EmittedFaces(
    ...
    /** The declared names of the interfaces the file faces: the AIDL binds these and no other. */
    val faced: Set<String> = emptySet(),
)
...
class FacesEmitter(private val model: Model, private val options: Options) {
    ...
    fun emit(): EmittedFaces {
        ...
        for (iface in model.interfacesList) {
            ...
            faced += iface.declared.declared
        }
        if (faced.isEmpty()) return EmittedFaces(path, null, emptyList(), warnings)
        addHelpers()
        file.addFileComment("Generated by %L from `%L`. Do not edit.", PLUGIN, model.name.dotted)
        return EmittedFaces(path, file.build().toString(), emptyList(), warnings, faced)
    }
